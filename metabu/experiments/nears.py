"""
Extractor de Vecinos usando el Modelo METABU.

Este módulo reutiliza la lógica de Task1 para extraer los vecinos más cercanos
de cualquier tarea usando las representaciones del modelo METABU.
...
from experiments.tasks import (
    ...
    get_target_representations,
    get_metabu_representations
)


class SimpleConfig:
    """Configuración simple para METABU."""
    def __init__(self, data_path, ndcg=15, seed=42):
        self.data_path = data_path
        self.seed = seed
        self.task = type('obj', (object,), {'ndcg': ndcg})()
        # Metafeatures básicas 'metabu' con sus 135 columnas
        # Se cargan todas de basic_representations.csv (excepto task_id)
        self.metafeature = type('obj', (object,), {
            'name': 'metabu',
            'basic_columns': self._get_basic_columns(data_path)
        })()
    
    @staticmethod
    def _get_basic_columns(data_path):
        """Obtener nombres de columnas de metafeatures básicas."""
        import os
        import pandas as pd
        
        csv_path = os.path.join(data_path, "basic_representations.csv")
        if os.path.exists(csv_path):
            df = pd.read_csv(csv_path)
            # Retornar todas las columnas excepto task_id
            cols = [c for c in df.columns if c != 'task_id']
            return ",".join(cols)
        else:
            # Fallback: columnas conocidas de METABU
            return "one_itemset,two_itemset,leaves,leaves_branch,leaves_corrob"


def get_neighbors_wrt_metabu_mf(test_task_id, pipeline, data_path, k=10, verbose=True):
    """
    Extrae los K vecinos más cercanos a una tarea usando el modelo METABU.
    
    Esta función reutiliza la lógica de Task1 para entrenar METABU y calcular
    ...
    # Configuración
    cfg = SimpleConfig(data_path=data_path)
    
    # Simulamos clase pipeline para compatibilidad con get_target_representations
    ...
    target_reprs = get_target_representations(pipeline=pipeline_cfg, path=data_path)
    ...
    basic_reprs = get_basic_representations(metafeature=cfg.metafeature, path=data_path)
    ...
    # Entrenar METABU (como en Task1)
    basic_reprs_metabu = get_metabu_representations(
        cfg=cfg,
    ...
    if verbose:
        print(f"  ✓ Representaciones METABU calculadas: {basic_reprs_metabu.shape}")
    ...
    # Calcular distancias euclidianas en espacio METABU
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extrae vecinos usando modelo METABU (reutiliza lógica de Task1)"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    print("\n" + "="*70)
    print("EXTRACTOR DE VECINOS - MODELO METABU")
    print("="*70)
    print(f"\nConfigración:")
