...
\begin{figure}[H]
    ...
    \label{fig:metafeatx_top_configurations}
\end{figure}


\textbf{Augmentación del benchmark.}
...
El algoritmo MetaFeatX se entrena sobre $p = 1000 \times n$ datasets de entrenamiento del benchmark, previamente aumentados mediante \emph{bootstrap} (ver sección anterior y Apéndice B). Las meta-features de MetaFeatX se construyen en un ``procedimiento de tres pasos'' ilustrado en la Figura~\ref{fig:metafeatx_pipeline}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{ilustracion.png} % ajusta la ruta de la imagen
...
\subsection{Extracción de Vecinos y Ranking de Pipelines}
\label{sec:neighbors_ranking}
...
\paragraph{Extracción de vecinos en el espacio MetaFeatX.}
Dado un dataset objetivo con identificador $t$, se calcula su representación MetaFeatX $\psi(x_t)$ y se mide su distancia euclidiana a todas las demás tareas del benchmark:
\[
d(t, i) = \| \psi(x_t) - \psi(x_i) \|_2.
\]
Los $k$ datasets con menor distancia (excluyendo $t$) se seleccionan como vecinos más cercanos. Este procedimiento se repite de manera independiente para cada pipeline considerado (por ejemplo, \texttt{adaboost}, \texttt{random\_forest}, \texttt{libsvm\_svc}), ya que la representación objetivo y el espacio de hiperparámetros dependen del pipeline.

\paragraph{Uso del mejor rendimiento histórico.}
Para cada vecino $i$ y pipeline $p$, se consulta el benchmark histórico y se extrae el mejor desempeño observado:
\[
a_{i}^{(p)} = \max_{\theta \in \Theta_i^{(p)}} \text{accuracy}(i, \theta).
\]
Se opta por el \emph{mejor} valor en lugar del promedio del top-$k$ de configuraciones para capturar el potencial máximo del pipeline en datasets similares, reduciendo la influencia de configuraciones subóptimas y alineándose con el objetivo de AutoML de identificar configuraciones de alto rendimiento.

\paragraph{Ponderación exponencial por vecindad.}
No todos los vecinos aportan la misma cantidad de información. Aquellos más cercanos en el espacio MetaFeatX son más relevantes. Por ello, se emplea una ponderación exponencial decreciente:
\[
...
\paragraph{Ranking relativo de pipelines.}
El score $S^{(p)}$ se calcula para cada pipeline de manera independiente. Finalmente, los pipelines se ordenan de mayor a menor score, produciendo un ranking relativo:
\[
...
\paragraph{Relación con MetaFeatX y AutoML.}
Este procedimiento es coherente con la filosofía de MetaFeatX: las meta-features aprendidas definen un espacio donde la proximidad refleja similitud en estructuras de rendimiento de hiperparámetros. El ranking resultante puede utilizarse para:
\begin{itemize}
    \item priorizar pipelines antes de la optimización,
    \item inicializar configuraciones en métodos como SMAC,
    \item o realizar selección de modelos informada por meta-aprendizaje.
\end{itemize}
...
\subsection{Análisis mediante diagramas de caja (boxplots)}

\paragraph{Motivación.}
En AutoML y Meta-learning no se evalúa el desempeño sobre un único dataset, sino sobre un conjunto heterogéneo de muchos datasets. Por lo tanto, la pregunta relevante no es \emph{``qué método tiene el mejor promedio''}, sino más bien:
\begin{quote}
\emph{``qué método funciona de manera confiable en una amplia variedad de datasets''.}
\end{quote}
Esta distinción es crucial, ya que en escenarios reales el usuario desconoce a priori las características del dataset que enfrentará.

\paragraph{Métrica utilizada.}
Para cada dataset, el metalearner produce un ranking de configuraciones (combinaciones de algoritmo e hiperparámetros). La métrica \(\text{NDCG@}k\) cuantifica qué tan arriba en dicho ranking aparece la mejor configuración real. De este modo, cada dataset genera un valor escalar de NDCG@\(k\). En nuestro caso, al disponer de 64 datasets, se obtienen 64 valores de NDCG@\(k\) por método evaluado.

\paragraph{Qué representa el boxplot.}
El diagrama de caja resume la distribución de estos valores de NDCG@\(k\) y se interpreta de la siguiente forma:
\begin{itemize}
    \item La \textbf{línea central} corresponde a la mediana y representa el comportamiento de un dataset típico.
    \item La \textbf{caja} contiene el 50\% central de los datasets (entre el primer y tercer cuartil).
    \item Los \textbf{bigotes} indican el rango de variación considerado normal.
    \item Los \textbf{puntos fuera de los bigotes} (outliers) corresponden a datasets particularmente difíciles, para los cuales el método no logró producir un buen ranking de configuraciones.
\end{itemize}

A diferencia de un simple promedio, el boxplot permite evaluar simultáneamente el rendimiento y la estabilidad del método a través de múltiples tareas.
...
\begin{itemize}
    \item en una posición más alta,
    \item con una caja más compacta,
    \item y con menos outliers,
\end{itemize}
entonces se puede concluir que MetaFeatX no solo obtiene un mayor rendimiento medio, sino que además es más consistente a lo largo de los diferentes datasets. Esto constituye evidencia de una \emph{transferencia de conocimiento real} entre tareas.

\paragraph{Relevancia práctica.}
En aplicaciones reales de AutoML, el usuario no sabe de antemano qué dataset enfrentará, por lo que necesita un sistema que funcione bien ``casi siempre''. El uso de boxplots permite visualizar precisamente esta propiedad, evaluando no solo cuán bueno es un método en promedio, sino también cuán confiable es frente a la variabilidad entre datasets.


\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{boxplot_task1_by_pipeline.jpeg}
    \caption{Diagramas de caja de NDCG@\(k\) para cada pipeline y cada tipo de metafeature. Se muestra la distribución del rendimiento a lo largo de todos los datasets considerados.}
    \label{fig:boxplot_task1}
\end{figure}

\paragraph{Discusión de resultados.}
En el caso del pipeline basado en \emph{AdaBoost}, se observa que, a medida que aumenta el valor de \(k\), el boxplot correspondiente a MetaFeatX presenta una clara compactación, lo que indica una mayor estabilidad en el rendimiento a través de los distintos datasets. Paralelamente, los valores de NDCG alcanzados por MetaFeatX son sistemáticamente superiores a los obtenidos mediante las metafeatures predefinidas de AutoSklearn, así como por Landmark y SCOT, lo que refleja una mejor capacidad para ordenar las configuraciones según su desempeño real.

Este comportamiento no es exclusivo de AdaBoost, sino que se reproduce de manera consistente en los demás pipelines evaluados. En conjunto, estos resultados indican que MetaFeatX no solo logra mayores valores promedio de NDCG, sino que también ofrece recomendaciones más estables y confiables a lo largo de una amplia variedad de datasets. Por lo tanto, puede concluirse que MetaFeatX cumple efectivamente su objetivo principal: proporcionar rankings de configuraciones de mayor calidad que los obtenidos mediante metafeatures diseñadas manualmente.

\subsection{Curva de aprendizaje basada en NDCG@20}

\paragraph{Motivación.}
La curva de NDCG@20 en función del tamaño del conjunto de entrenamiento mide directamente la \emph{transferencia de conocimiento entre tareas}, es decir, cuánta experiencia previa necesita el metalearner para producir recomendaciones útiles en un dataset nunca visto. A diferencia de métricas de error clásicas, esta curva evalúa la capacidad del sistema para generalizar a nuevas tareas a partir de experiencias pasadas.

La elección de NDCG@\(k\) con \(k=20\) responde a que, en este contexto, no solo interesa que el sistema identifique una única buena configuración, sino que ordene correctamente el espacio completo de configuraciones. Un valor elevado de NDCG@20 indica que las mejores configuraciones reales aparecen de manera consistente entre las primeras posiciones del ranking recomendado por el metalearner.

\paragraph{Construcción de la curva.}
Para cada dataset \(d \in D\), se aplica un esquema de \emph{leave-one-out} a nivel de tareas: el dataset \(d\) se reserva como conjunto de prueba meta, mientras que el resto \(D \setminus \{d\}\) se divide en un subconjunto de entrenamiento \(D_{\text{train}}\) y un subconjunto de validación \(D_{\text{val}}\). El metalearner se entrena únicamente sobre \(D_{\text{train}}\), se regulariza y ajusta usando \(D_{\text{val}}\), y finalmente se evalúa sobre el dataset nunca visto \(d\).

Este procedimiento se repite para distintos tamaños \(|D_{\text{train}}|\), generando así un conjunto de valores de NDCG@20 que, al promediarse sobre todos los datasets, define la curva de aprendizaje. De este modo, la curva refleja cómo evoluciona la capacidad de recomendación del metalearner a medida que aumenta la cantidad de tareas disponibles durante su entrenamiento.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{training_vs_test.jpeg}
    \caption{Curva de aprendizaje basada en NDCG@20. Se muestra el rendimiento medio del metalearner en los conjuntos de entrenamiento y validación a medida que aumenta el tamaño del conjunto de entrenamiento meta.}
    \label{fig:learning_curve}
\end{figure}

\paragraph{Discusión de resultados.}
Si bien se aprecia un sobreajuste marcado en el conjunto de entrenamiento, los valores obtenidos en el conjunto de validación permanecen sistemáticamente por encima del baseline de referencia (AutoSklearn). Esto indica que, a pesar del gap entre entrenamiento y validación, el metalearner logra generalizar conocimiento útil hacia datasets no vistos.

El crecimiento progresivo de la curva de validación ratifica la hipótesis de que el modelo está aprendiendo una \emph{topología inducida por las representaciones objetivo de los datasets}, lo que se traduce en recomendaciones de configuraciones cada vez más precisas a medida que aumenta el tamaño del conjunto de entrenamiento meta. 

Cabe destacar que estos resultados son susceptibles de mejora mediante la incorporación de un mayor número de datasets al conjunto global (actualmente limitado a 64) y a través de un ajuste más fino de los hiperparámetros del metalearner durante la fase de entrenamiento y validación. Dicho ajuste se realiza exclusivamente sobre las particiones \(D_{\text{train}}\) y \(D_{\text{val}}\), sin involucrar al dataset \(d\) reservado como prueba, preservando así la validez del protocolo de evaluación.
...
La evaluación experimental presentada en la Sección~\ref{sec:experimentation} proporciona una validación exhaustiva y multifacética del sistema de meta-learning propuesto. A través de un protocolo riguroso de validación cruzada sobre tareas, múltiples métricas complementarias y análisis estadísticos formales, los resultados permiten extraer conclusiones sólidas tanto sobre el comportamiento individual de los componentes del sistema como sobre su interacción conjunta.

\subsection{MetaFeatX como representación topológica del espacio de tareas}

Los resultados de la Tarea 1 (Sección 4.2) demuestran de manera consistente que MetaFeatX captura de forma más fiel la topología inducida por el rendimiento real de los algoritmos que los conjuntos de meta-características diseñadas manualmente. La superioridad sistemática en NDCG@k para todos los valores de $k$ considerados, así como la estabilidad observada en los boxplots (Figura~\ref{fig:boxplot_task1}), indican que la representación aprendida preserva relaciones semánticamente relevantes entre datasets.

Este comportamiento sugiere que el uso de Transporte Óptimo Fused Gromov-Wasserstein permite integrar de forma efectiva información estructural y distributiva, superando las limitaciones inherentes a las meta-características estáticas. El hecho de que la ventaja de MetaFeatX aumente con valores mayores de $k$ refuerza la hipótesis de que la representación aprendida no solo identifica vecinos inmediatos, sino que modela correctamente regiones completas del espacio de problemas.

\subsection{Generalización y transferencia entre tareas}

La curva de aprendizaje basada en NDCG@20 (Sección 4.11) proporciona evidencia directa de la capacidad de transferencia de MetaFeatX. A pesar del sobreajuste esperado en el conjunto de entrenamiento —inevitable dada la limitada cantidad de tareas disponibles—, el desempeño sostenido en validación por encima del baseline de AutoSklearn indica que el modelo aprende regularidades transferibles entre tareas.

Este resultado es particularmente relevante en escenarios reales de AutoML, donde el sistema debe enfrentarse a datasets completamente nuevos. La mejora progresiva del rendimiento en validación a medida que aumenta el número de tareas de entrenamiento confirma que MetaFeatX aprende una topología latente del espacio de problemas, en lugar de memorizar características específicas de los datasets observados.

\subsection{AutoML sin modelo explícito de rendimiento}

Los resultados de la Tarea 2 muestran que incluso en ausencia de un modelo de rendimiento explícito, MetaFeatX proporciona información suficiente para guiar eficazmente el muestreo inicial de configuraciones. La mejora consistente frente a muestreadores uniformes y a meta-características clásicas indica que la información de vecindario inducida por MetaFeatX es altamente informativa.

Este hallazgo es significativo porque demuestra que la representación aprendida es útil por sí misma, independientemente del método de optimización posterior. En otras palabras, MetaFeatX no solo funciona como un complemento de FSBO, sino que constituye un módulo autónomo de meta-learning aplicable a distintos esquemas de AutoML.

\subsection{Sinergia entre MetaFeatX y FSBO}

La evaluación del sistema integrado (Sección 4.4) pone de manifiesto una clara sinergia entre MetaFeatX y FSBO. Mientras MetaFeatX aporta una inicialización informada y una selección más precisa de regiones prometedoras del espacio de búsqueda, FSBO explota esta información para acelerar la convergencia mediante transferencia de conocimiento entre tareas.

Esta complementariedad se observa con especial claridad en Random Forest, donde se obtiene una reducción significativa del Normalized Regret $(9-11\%)$ respaldada por tests estadísticos rigurosos (Friedman, Nemenyi y Wilcoxon). La mejora más pronunciada en las primeras iteraciones confirma que el conocimiento transferido es particularmente valioso durante la fase de exploración temprana, cuando las decisiones iniciales tienen un impacto desproporcionado en el rendimiento final.

\subsection{Impacto de la complejidad del espacio de búsqueda}

Las diferencias observadas entre algoritmos (Sección 4.12) revelan una relación clara entre la efectividad del transfer learning y la complejidad del espacio de hiperparámetros. En espacios de dimensionalidad moderada y comportamiento más regular, como Random Forest y AdaBoost, FSBO logra explotar patrones compartidos entre tareas de manera eficiente. En contraste, en espacios altamente complejos como AutoSklearn, la ganancia relativa es menor, aunque consistente.

Este comportamiento sugiere que, si bien el enfoque propuesto es general, su mayor beneficio se obtiene en escenarios donde existe una estructura subyacente transferible entre tareas, y donde el presupuesto evaluativo es limitado.

\subsection{Robustez, estabilidad e interpretabilidad}

Los análisis de sensibilidad (Sección 4.9) muestran que MetaFeatX es robusto frente a variaciones razonables de sus hiperparámetros principales, lo cual es una propiedad deseable en sistemas AutoML. Asimismo, el análisis de interpretabilidad revela que la representación aprendida asigna pesos significativos a meta-características coherentes con el comportamiento esperado de cada algoritmo, proporcionando una explicación intuitiva de sus decisiones.

En conjunto, estos resultados refuerzan la idea de que MetaFeatX no actúa como una caja negra opaca, sino como un mecanismo de aprendizaje estructurado que puede ser analizado y comprendido.
...
En este trabajo se ha presentado un sistema de AutoML basado en meta-learning que combina el aprendizaje de meta-características mediante Transporte Óptimo (MetaFeatX) con un enfoque de optimización bayesiana con transferencia de conocimiento (FSBO). A través de una evaluación experimental exhaustiva y estadísticamente validada, se ha demostrado que esta combinación permite abordar de manera efectiva el problema de \emph{cold-start} en AutoML.

Los resultados experimentales confirman que MetaFeatX aprende representaciones de datasets que capturan la topología relevante del espacio de problemas, superando de forma consistente a meta-características diseñadas manualmente en tareas de identificación de similitud entre datasets. Esta capacidad se traduce en recomendaciones más precisas, estables y transferibles, como se evidencia en las métricas NDCG@k, los boxplots y las curvas de aprendizaje.

Asimismo, se ha demostrado que FSBO puede explotar eficazmente la información proporcionada por MetaFeatX para acelerar la optimización de hiperparámetros, particularmente en espacios de búsqueda de complejidad moderada. La reducción significativa del Normalized Regret y la mejora en la eficiencia de convergencia validan el valor práctico del transfer learning en escenarios de presupuesto evaluativo limitado.

Un aspecto central de este trabajo es la demostración de que la integración de MetaFeatX y FSBO produce un sistema cuyo rendimiento supera al de cada componente por separado. MetaFeatX proporciona una inicialización informada y una estructuración del espacio de tareas, mientras que FSBO refina esta información mediante exploración adaptativa. Esta sinergia resulta especialmente relevante en contextos reales de AutoML, donde el número de evaluaciones es restringido y la generalización a nuevos datasets es crítica.

Finalmente, los análisis de sensibilidad, interpretabilidad y estabilidad operacional indican que el sistema propuesto es robusto y confiable, lo que refuerza su viabilidad práctica. Como líneas futuras de trabajo, se plantea ampliar el benchmark de tareas, evaluar el sistema con datos reales de rendimiento en OpenML, e integrar métodos de optimización más escalables y modelos de transferencia más expresivos.

En conjunto, este trabajo establece un marco sólido para el uso de meta-learning basado en representaciones aprendidas como pilar fundamental de sistemas AutoML adaptativos y eficientes.


%-------------------------------------------------
\newpage
\addcontentsline{toc}{section}{Anexos} % <-- esto lo agrega al índice
...
\begin{itemize}
    ...
    \item Información sobre el problema AutoML proporcionada por el enfoque: dimensionalidad intrínseca (\hyperref[appendix:F1]{Apéndice F.1}) y visualización de los nichos de los algoritmos ML considerados (\hyperref[appendix:F2]{Apéndice F.2})
    \item Análisis de sensibilidad de la dimensión $d$ (\hyperref[appendix:G]{Apéndice G})
    \item Curvas de desempeño de la Tarea 2 (\hyperref[appendix:H]{Apéndice H})
\end{itemize}

\newpage
\appendix

\section*{Augmentaci\'on del benchmark OpenML}
\label{appendix:A}

La visualización del benchmark ampliado mediante \emph{t-SNE} sobre el espacio de meta-features básicas muestra que los conjuntos de datos generados por \emph{bootstrapping} a partir de un mismo dataset original forman clústeres compactos alrededor de este. Este comportamiento es esperado, ya que las meta-features diseñadas manualmente son estables frente a pequeñas variaciones estocásticas de los datos.

Asimismo, los clústeres asociados a distintos datasets de OpenML permanecen claramente separados entre sí, incluso al variar el parámetro de \emph{perplexity} en un amplio rango. Este resultado sugiere que el benchmark original de OpenML cubre de manera dispersa el espacio de meta-features, y que la ampliación mediante \emph{bootstrapping} permite densificar localmente dicho espacio sin introducir solapamientos artificiales entre datasets no relacionados.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{cluster_plot2.png}
    \caption{Visualización 2D mediante t-SNE de los conjuntos de datos de OpenML en el espacio de representación básica ,junto con sus ampliaciones generadas por bootstrapping. Solo se muestran algunos nombres de datasets para facilitar la legibilidad..}
    \label{fig:bootstrap}
\end{figure}
...
\newpage
\section*{La estabilidad de la dimensi\'on intr\'inseca}
\label{appendix:F1}
...
\begin{figure}[h]
    ...
    \caption{Dimensión intrínseca del espacio de datasets con respecto a los algoritmos de ML Adaboost, RandomForest, SVM y AutoSkLearn, en función de la fracción de datasets considerados en OpenML.}
    \label{fig:intrinsic_table}
\end{figure}



En la tabla ~\ref{fig:intrinsic_table} investigamos cómo varía la dimensión intrínseca al considerar distintos números de conjuntos de datos en OpenML. Se observa que la dimensión intrínseca tiende a aumentar con el número de conjuntos de datos considerados, especialmente en el caso de SVM y, en menor medida, en Random Forest. Esto sugiere que las configuraciones de hiperparámetros exploradas en el benchmark de OpenML para estos algoritmos no muestrean de manera suficientemente adecuada (las regiones buenas del) espacio de configuraciones.

\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{grafica_intrinsi.png}
    \caption{Dimensión intrínseca del espacio de datasets con respecto a los algoritmos de ML Adaboost, RandomForest, SVM y AutoSkLearn, en función de la fracción de datasets considerados en OpenML.}
    \label{fig:intrinsic_graphicx}
\end{figure}


\newpage
\subsection*{Interpretación: impacto de las meta-features HC en el rendimiento del algoritmo de aprendizaje}
\label{appendix:F2}

Las meta-features \textit{MetaFeatX} se construyen a partir de las meta-features iniciales definidas manualmente (HC meta-features) mediante un mapeo lineal aprendido $\psi$, el cual depende del algoritmo de aprendizaje considerado. Este mapeo proyecta las representaciones básicas de los datasets a un espacio latente de menor dimensión que captura las propiedades relevantes para el rendimiento del algoritmo.

Con el objetivo de interpretar el impacto de las meta-features originales sobre el rendimiento de cada algoritmo, se analiza la importancia de dichas características a través de un análisis de componentes principales (PCA). Para ello, se considera la matriz $U$ obtenida mediante \textit{Multidimensional Scaling} (MDS) aplicada a las representaciones objetivo de todos los datasets, tal como se describe en la Sección~3.2.

El primer componente principal de $U$ representa la dirección de máxima variabilidad entre los datasets en el espacio MetaFeatX. Se identifica entonces la dimensión $j^*$ que más contribuye a dicho componente, y se utiliza el mapeo lineal $\psi$ para estimar la contribución de cada meta-feature original a esta dimensión dominante. Formalmente, la importancia de la $k$-ésima meta-feature inicial con respecto a un algoritmo de aprendizaje $A$ se define como el valor absoluto del coeficiente correspondiente $\lvert \psi_{j^*,k} \rvert$.

Este procedimiento permite cuantificar qué propiedades de los datasets son más determinantes para el rendimiento de un algoritmo específico. Además, facilita la comparación entre distintos algoritmos al representar cada meta-feature como un punto en el plano bidimensional $(i_A(k), i_B(k))$, donde $i_A(k)$ y $i_B(k)$ indican la importancia de la meta-feature $k$ para los algoritmos $A$ y $B$, respectivamente.
...
Asimismo, la importancia de las meta-features con respecto a Support Vector Machines y Random Forest se muestra en la Figura ~\ref{fig:Figura_8} (derecha). Las características relacionadas con la asimetría (skewness) —media y desviación estándar sobre todos los atributos— resultan significativamente más relevantes para Support Vector Machines que para Random Forest. En retrospectiva, no sorprende que las meta-features asociadas a las posibles dificultades en la inversión de la matriz de Gram sean particularmente importantes para SVM.

\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{meta-feature-grafica.png}
    \caption{Importancia comparativa de las meta-features para Random Forest vs. AutoSkLearn (izquierda) y SVM (derecha). Las meta-features específicas de AutoSkLearn se indican porque su nombre comienza con una letra mayúscula.}
    \label{fig:Figura_8}
\end{figure}
...
La Tabla~\ref{fig:sensitivity_d} presenta el análisis de sensibilidad del método \textit{MetaFeatX} con respecto al número de meta-características $d$ en la Tarea 1, evaluado mediante la métrica NDCG@k. Los resultados permiten extraer dos conclusiones principales. En primer lugar, el mejor desempeño se obtiene, en la gran mayoría de los casos, cuando $d$ coincide con la dimensión intrínseca estimada del benchmark. En segundo lugar, la sensibilidad del rendimiento con respecto a variaciones en $d$ es moderada, lo que indica una alta robustez del método frente a la elección de este hiperparámetro.

En particular, la dimensión intrínseca estimada del benchmark OpenML es aproximadamente igual a 6 para AutoSkLearn, 8 para AdaBoost, 9 para Random Forest y 14 para Support Vector Machines. Para estos valores, MetaFeatX alcanza sistemáticamente los mayores valores de NDCG@k, superando o igualando el rendimiento obtenido con dimensiones menores o mayores.

Este comportamiento sugiere que la selección de $d$ basada en la dimensión intrínseca permite capturar de forma más adecuada la estructura latente del espacio de tareas, evitando tanto la pérdida de información asociada a dimensiones demasiado bajas como el ruido introducido por representaciones excesivamente altas. Además, la estabilidad observada en los resultados para distintos valores de $k$ refuerza la idea de que MetaFeatX no es altamente sensible a pequeñas desviaciones en la elección de $d$, lo cual es una propiedad deseable en escenarios prácticos de AutoML.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{sensibilidad.png}
    \caption{Análisis de sensibilidad de MetaFeatX respecto al número de meta-características $d$ en la Tarea 1, medido mediante NDCG@k para distintos algoritmos.}
    \label{fig:sensitivity_d}
\end{figure}

\section*{Curvas de rendimiento de la Tarea 2}
\label{appendix:H}

Además de los resultados de ranking presentados en la Figura~3b,
en esta sección se muestran las curvas de rendimiento correspondientes a la Tarea 2,
evaluadas sobre 10 conjuntos de datos representativos del benchmark OpenML CC-18.

Estas curvas permiten observar no solo el orden relativo de los métodos, sino también la evolución
temporal de su desempeño absoluto a lo largo de las iteraciones de búsqueda. En particular, se
consideran los siguientes algoritmos base:

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{14_curve.png}
    \caption{}
    \label{fig:curve14}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{219_curve.png}
    \caption{}
    \label{fig:curve219}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{3022_curve.png}
    \caption{}
    \label{fig:curve3022}
\end{figure}
